...
export default {
  ...
  methods: {
    goEvalution() {
      this.$router.push('/parent/goparentevaluation');
    },
    goEvalutionDetail() {
      this.$router.push('/parent/parentevaluationdetail');
    },
  },
  created() {},
  async mounted() {
    this.noEvaluationList = (await getEvaluation(100, 0, 1, 0)).data;

    this.evaluationList = (await getEvaluation(100, 0, 1, 1)).data;
  },
};
</script>
